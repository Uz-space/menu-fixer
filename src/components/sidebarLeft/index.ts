...
export class AppSidebarLeft extends SidebarSlider {
  ...
  private createHomeIndicator() {
    ...
    // A touch gesture is followed by a burst of compatibility mouse events
    // (mousedown / mouseup / click) fired on whatever sits under the finger.
    // Since `CLICK_EVENT_NAME` is `mousedown` on touch devices, that burst
    // reaches the menu's own outside-click handler right after the menu was
    // opened by the swipe and closes it again instantly — so the menu looked
    // like it never opened at all. Swallow the burst for a short window.
    const swallowCompatMouseEvents = () => {
      const types = ['mousedown', 'mouseup', 'click'];
      const swallow = (e: Event) => {
        if(e.isTrusted) cancelEvent(e);
      };

      const stop = () => {
        clearTimeout(timeout);
        types.forEach((type) => document.removeEventListener(type, swallow, true));
      };

      types.forEach((type) => document.addEventListener(type, swallow, true));
      const timeout = window.setTimeout(stop, 700);
    };
